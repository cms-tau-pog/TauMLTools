...
class MyGNN(tf.keras.Model):
    ...
    def __init__(self, dl_config):
        ...
        for i in range(self.n_dense_layers-1):
            ...
            self.dense_acti.append(tf.keras.layers.Activation("relu", name='dense_acti{}'.format(i)))
            if(self.dropout_rate > 0):
        ...
        self.dense2 = tf.keras.layers.Dense(n_last, kernel_initializer="he_uniform", activation='sigmoid',
...
def main(cfg: DictConfig) -> None:
    ...
    with mlflow.start_run(**run_kwargs) as active_run:
        ...
        compile_model(model, dl_config["SetupNN"]["learning_rate"])
        ...
        mlflow.log_artifact(to_absolute_path("Training_SNNtag_v0.py"), 'input_cfg')
        mlflow.log_artifact(to_absolute_path("../commonReco.py"), 'input_cfg')
        mlflow.log_artifacts('.hydra', 'input_cfg/hydra')
        mlflow.log_artifact('Training_SNNtag)v0.log', 'input_cfg/hydra')
