...
pt_res_obj_rel = MyResolution('pt_res_rel' , 0 ,True)
pt_res_obj     = MyResolution('pt_res' , 0 ,False)
m2_res_obj     = MyResolution('m^2_res', 1 ,False)
...
def my_mse_pt(y_true, y_pred):
    def_mse = 0.022759110487849007
    w = 1/def_mse
    return w*tf.square((y_true[:,0] - y_pred[:,0]) / y_true[:,0])
...
class CustomMSE(tf.keras.losses.Loss):
    ...
    def __init__(self, name="custom_mse", **kwargs):
        ...
        if CustomMSE.mode != "p4":
            raise ValueError('Only p4 is available in CustomeMSE')
