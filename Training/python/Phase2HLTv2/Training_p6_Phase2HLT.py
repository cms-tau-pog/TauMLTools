...
print("Using TensorFlow version '%s' with Keras version '%s'." % (tf.__version__, keras.__version__))
...
TauLosses.SetSFs(TauLoss_SF_e, TauLoss_SF_mu, TauLoss_SF_tau, TauLoss_SF_jet)
print(TauLosses.Le_sf, TauLosses.Lmu_sf, TauLosses.Ltau_sf, TauLosses.Ljet_sf)
print("netConf_full = %s" % netConf_full)
model = create_model(netConf_full)
compile_model(model, 1e-3)

loader = DataLoader(args.input, netConf_full, 100, 2000,
                    validation_size=50000, max_queue_size=40, n_passes=-1, return_grid=True)
print(loader.file_entries)
print(loader.total_size, loader.data_size, loader.validation_size)

fit_hist = run_training('step{}'.format(1), args.model, loader, 0, 10)
