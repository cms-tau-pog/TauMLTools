...
import argparse
parser = argparse.ArgumentParser(description='Run DeepTau training.')
parser.add_argument('--input', required=True, type=str, help="Input file")
parser.add_argument('--model', required=True, type=str, help="Name of trained DNN model")
args = parser.parse_args()

# CV: check that input file exists
...
def create_cell_model(net_config, loc):
    ...
    model = Model(input_layers, prev_layer, name=args.model)
    return model

inner_model = create_cell_model(netConf_full, 'inner')
print("Architecture of 'inner' model:")
inner_model.summary()
inner_model.load_weights(args.input, by_name=True)
outputFileName_inner = os.path.basename(args.input).replace(".h5", "_inner.h5")
print("Writing 'inner' model to file = '%s'." % outputFileName_inner)
inner_model.save(outputFileName_inner)

outer_model = create_cell_model(netConf_full, 'outer')
print("Architecture of 'outer' model:")
outer_model.summary()
outer_model.load_weights(args.input, by_name=True)
outputFileName_outer = os.path.basename(args.input).replace(".h5", "_outer.h5")
print("Writing 'outer' model to file = '%s'." % outputFileName_outer)
outer_model.save(outputFileName_outer)
...
def create_core_model(net_config):
    ...
    print(input_layers)
    model = Model(input_layers, softmax_output, name=args.model)
    return model

core_model = create_core_model(netConf_full)
print("Architecture of 'core' model:")
core_model.summary()
core_model.load_weights(args.input, by_name=True)
outputFileName_core = os.path.basename(args.input).replace(".h5", "_core.h5")
print("Writing 'core' model to file = '%s'." % outputFileName_core)
core_model.save(outputFileName_core)
