...
def create_df(path_to_preds, pred_samples, tau_type_to_select, selection,
              ...
              **kwargs):
    df = []
    pred_paths = glob(path_to_preds)
    if len(pred_paths) != 1:
        raise Exception(f"Found {len(pred_paths)} predictions in {path_to_preds}")
    path_to_preds = to_absolute_path(pred_paths[0])
    ...
    for sample_name, sample_cfg in pred_samples.items():
        ...
        df_sample['weight'] = 1.
        if sample_cfg.get('reweight_to_lumi'):
