  ...
  ,roundOff_(pset.getUntrackedParameter("roundOff",(double)(0.01)))
{
  ...
  produces<double>("TauSpinnerWTEven").setBranchAlias("TauSpinnerWTEven");
  produces<double>("TauSpinnerWTOdd").setBranchAlias("TauSpinnerWTOdd");
  produces<double>("TauSpinnerWTMM").setBranchAlias("TauSpinnerWTMM");
...
void TauPOGSpinner::initialize(){
  ...
  if(!isTauSpinnerConfigure){
    ...
    TauSpinner::setHiggsParametersTR(-1., 1., 0.0, 0.0); //This is scalar Higgs
...
void TauPOGSpinner::produce( edm::Event& e, const edm::EventSetup& iSetup){
  ...
  if(MotherPDGID_<0 || abs(X.pdgid())==MotherPDGID_){
	...
	for(unsigned int i=0; i<tau_daughters2.size();i++){
	  ...
	  //Go from CP Odd...
          TauSpinner::setHiggsParametersTR(1., -1., 0.0, 0.0);	
          WTodd = TauSpinner::calculateWeightFromParticlesH(X, tau, tau2, tau_daughters,tau_daughters2);
	  //...to Maximum Mixing case...
          double theta=TMath::Pi()/4.;
          TauSpinner::setHiggsParametersTR(-cos(2*theta),cos(2*theta) ,-sin(2*theta),-sin(2*theta));
          WTmm = TauSpinner::calculateWeightFromParticlesH(X, tau, tau2, tau_daughters,tau_daughters2);
	  //...and back to CP Even.
	  TauSpinner::setHiggsParametersTR(-1., 1., 0.0, 0.0);
  ...
  *TauSpinnerWeightisValid =isValid;
  ...
  // even weight
  std::unique_ptr<double> TauSpinnerWeightEven(new double);
  *TauSpinnerWeightEven =WT;
  e.put(std::move(TauSpinnerWeightEven),"TauSpinnerWTEven");

  // maximum mixing weight
  std::unique_ptr<double> TauSpinnerWeightOdd(new double);
  *TauSpinnerWeightOdd =WTodd;
  e.put(std::move(TauSpinnerWeightOdd),"TauSpinnerWTOdd");

  // odd weight
  std::unique_ptr<double> TauSpinnerWeightMM(new double);
  *TauSpinnerWeightMM =WTmm;
  e.put(std::move(TauSpinnerWeightMM),"TauSpinnerWTMM");
...
int TauPOGSpinner::readParticlesfromReco(edm::Event& e,SimpleParticle &X,SimpleParticle &tau,SimpleParticle &tau2,
  ...
  for(reco::GenParticleCollection::const_iterator itr = genParticles->begin(); itr!= genParticles->end(); ++itr){
    ...
    if(pdgid==24 || pdgid==37 || pdgid ==25 || pdgid==36 || pdgid==22 || pdgid==23 ){
      ...
      for(unsigned int i=0; i<hx->numberOfDaughters(); i++){
	const reco::Candidate *dau=hx->daughter(i);
